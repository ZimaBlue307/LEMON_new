...
def is_layer_in_activation_list(layer):
    import mindspore
    activation_list = [mindspore.nn.layer.activation.Softmin, mindspore.nn.layer.activation.Softmax,
                       mindspore.nn.layer.activation.LogSoftmax,
                       mindspore.nn.layer.activation.ReLU, mindspore.nn.layer.activation.ReLU6,
                       mindspore.nn.layer.activation.RReLU,
                       mindspore.nn.layer.activation.SeLU, mindspore.nn.layer.activation.SiLU,
                       mindspore.nn.layer.activation.Tanh,
                       mindspore.nn.layer.activation.Tanhshrink, mindspore.nn.layer.activation.Hardtanh,
                       mindspore.nn.layer.activation.GELU,
                       mindspore.nn.layer.activation.FastGelu, mindspore.nn.layer.activation.Sigmoid,
                       mindspore.nn.layer.activation.Softsign,
                       mindspore.nn.layer.activation.PReLU, mindspore.nn.layer.activation.LeakyReLU,
                       mindspore.nn.layer.activation.HSigmoid,
                       mindspore.nn.layer.activation.HSwish, mindspore.nn.layer.activation.ELU,
                       mindspore.nn.layer.activation.LogSigmoid,
                       mindspore.nn.layer.activation.LRN, mindspore.nn.layer.activation.SoftShrink,
                       mindspore.nn.layer.activation.HShrink,
                       mindspore.nn.layer.activation.CELU, mindspore.nn.layer.activation.Threshold,
                       mindspore.nn.layer.activation.Mish
                       ]
...
def is_layer_in_activation_list_without_softmax(layer):
    import mindspore
    activation_list = [mindspore.nn.layer.activation.Softmin, mindspore.nn.layer.activation.LogSoftmax,
                       mindspore.nn.layer.activation.ReLU, mindspore.nn.layer.activation.ReLU6,
                       mindspore.nn.layer.activation.RReLU,
                       mindspore.nn.layer.activation.SeLU, mindspore.nn.layer.activation.SiLU,
                       mindspore.nn.layer.activation.Tanh,
        ...
        if len(val.shape) > 1:
            ...
            for output_channel in mutate_output_channels:
                ...
                val = np.reshape(copy_list,
                                 (num_of_output_channels, num_of_input_channels, filter_width, filter_height))
...
def _LS_scan(model):
    ...
    shape_dict = {}
    for i, layer in enumerate(layers):
        if is_layer_in_activation_list(layer):
            break
        if isinstance(layer.input, list) and len(layer.input) > 1:
            continue
...
def GF_mut(model, mutation_ratio, distribution='normal', STD=0.1, lower_bound=None, upper_bound=None):
    ...
    layer_name, layer = utils.ModelUtils.get_layer(model, chosed_index)
...
def ARem_mut(model, mutated_layer_indices=None):
    ...
    # in lemon raw, the activation of last layer should not be removed
    # but in ms, activation itself is a layer.
    ...
    print(type(mutated_layer_indices))
    # mutated_layer_indices是一个array，其中的元素在[0, x)中，各随机出现一次

    for i in mutated_layer_indices:
        layer = layers[i]
        if is_layer_in_activation_list_without_softmax(layer):
...
def ARep_mut(model, new_activations=None, mutated_layer_indices=None):
    ...
    layers = model.cells_and_names()
    len_layers, ly_map = utils.ToolUtils.get_layers(ARep_model)
    ...
    for i in mutated_layer_indices:
        ...
        layer_name, layer = utils.ModelUtils.get_layer(ARep_model, i)
        if is_layer_in_activation_list_without_softmax(layer):
...
def LA_mut(model, new_layers=None, mutated_layer_indices=None):
    ...
    if model.__class__.__name__ == 'Sequential':
        ...
        new_model = utils.ModelUtils.functional_model_operation(LA_model, operation={
            LA_model.layers[layer_index_to_insert].name: layer_addition})
